...
/*   Updated: 2023/01/26 15:51:43 by bsoubaig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * This function is a replicated version of the libft function ft_strlen
 * 
 * @param s 		- The string to be counted
 * @return int 		- The number of characters in the string
 */
int	ft_strlen_fork(const char *s)
{
	int	i;
...
char	*ft_strjoin_fork(char const *s1, char const *s2)
{
	...
	str = (char *) malloc(sizeof(char) * \
		(ft_strlen_fork(s1) + ft_strlen_fork(s2) + 1));
	if (!str)
		return (NULL);
	i = 0;
	j = 0;
